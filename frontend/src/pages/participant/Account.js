...
class AccountPage extends Component {
    ...
    render() { 
        const {groupsMember, fetching } = this.props;
        if (fetching) {
            return <Spinner/>
        }
        ...
        return (
            <div>
                <Spinner/>
                {Object.keys(groupsMember).map((keyName, i) => (
                    <li className="travelcompany-input" >
                    {/* {keyName.id_participant.map((item) =>{
                        <p>{item._id}</p>
                    })} */}
                        <span className="input-label">key: {i} Name: {groupsMember[keyName]}</span>
                    </li>
                ))}
            </div>
        );
